...
db_open(const char *pathname, int oflag, ...)
{
	...
	if ((oflag & (O_CREAT | O_TRUNC)) == (O_CREAT | O_TRUNC)) {
		...
		if (statbuff.st_size == 0) {
			 ...
			 * ptr with a value of 0. The +1 is for the free
...
_db_alloc(int namelen)
{
	...
	if ((db = calloc(1, sizeof(DB))) == NULL)
		err_dump("_db_alloc: calloc error fot DB");
	db->idxfd = db->datfd = -1;
...
_db_readptr(DB *db, off_t offset)
{
	...
	asciiptr[PTR_SZ] = 0;	/* null terminated */
...
_db_readidx(DB *db, off_t offset)
{
	 ...
	 * We still need to lseek to record the current offset.
	 */
	...
	if (db->idxlen < IDXLEN_MIN || db->idxlen > IDXLEN_MAX)
	  	err_dump("_db_readidx: idx invalid length");
	
	/* 
	 * Noe read the actual index record. We read it into the key
	...
	*ptr2++ = 0;	/* replace SEP with null */
	...
	if ((db->datlen = atol(ptr2)) <= 0 || db->datlen > DATLEN_MAX)
		err_dump("_db_readidx: dat invalid length");
...
_db_dodelete(DB *db)
{
	...
	*ptr = 0;	/* null terminate for _db_writedat */
	...
	/* 
	 * Read the free list pointer. Its value become the chain
	 * ptr field of the deleted index record. This means
 ...
 * this function to set the datoff and datlen fields in the DB
 * structure, which we need to write the index records
 */
static void
_db_writeidx(DB *db, const char *key, off_t offset, int whence, off_t ptrval)
{
	...
	/*
	 * If we're appending, we have to lock before doing the lseek
	 * and write to make the two an atomic opration. If we're
	 * overwriting an existing record, we don't have to lock
	 */
	...
	if (writev(db->idxfd, &iov[0], 2) != PTR_SZ + IDXLEN_SZ + len)
		err_dump("_db_writeidx: writev error of data record");
...
db_store(DBHANDLE h, const char *key, const char *data, int flag)
{
	...
	/*
	 * _db_find_and_lock calculate which hash table this new record
	...
	if (_db_find_and_lock(db, key, 1) < 0)	{	/* record not found */
		if (flag == DB_REPLACE) {
			rc = -1;
			db->cnt_storerr++;
			errno = ENOENT;		/* error, record does not exits */
			goto doreturn;
		}
		 ...
		 * key equals the existing key, but, we need to chech if
...
doreturn:	/* unlock hash chainn locked by _db_find_and_lock */
	if (un_lock(db->idxfd, db->chainoff, SEEK_SET, 1) < 0)
...
_db_findfree(DB *db, int keylen, int datlen)
{
	...
	if (offset == 0) {
		 ...
		 * the file list. We set this chain ptr to db->ptrval
 ...
 * We just step our way through the index file, ignoring deteled
